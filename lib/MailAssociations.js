...
module.exports = (function() {
    ...
    MailAssociations.prototype.saveMail = function(mail, callback) {
        ...
        //mail.createdAt = mail.updatedAt = new Date().getTime();
    ...
    MailAssociations.prototype.cancelScheduledMail = function(id, callback) {
        this.Mail.findOneAndUpdate({_id: id, status: 'approved'}, {status: 'canceled', updatedAt: new Date().getTime()}, callback);
    };
    MailAssociations.prototype.updateStatusMail = function(id, status, callback) {
        this.Mail.findOneAndUpdate({_id: id}, {status: status, updatedAt: new Date().getTime()}, callback);
    };
